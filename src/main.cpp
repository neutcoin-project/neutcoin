...
#if defined(NDEBUG)
# error "Neutcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb7fa7755200cd58dd9ce44439961f75e46580ef9b31811d6ec67187ee0231e32");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Neutcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Neutcoin Signed Message:\n";
...
{
    // Neutcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Neutcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Neutcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Neutcoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Neutcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Neutcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb7fa7755200cd58dd9ce44439961f75e46580ef9b31811d6ec67187ee0231e32");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "12/24/2017 BTC Dips After Being At Over 25k CAD";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8ejvggh34jv3vhjbbby9b3498yry748738oy7cfe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1514156353;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1366147;
        ...
        assert(block.hashMerkleRoot == uint256("0x951155ffb0d181bbe117c8869eb8de635e5f7276d0841cac06b6ed7838f3d557"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Neutcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NeutcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NeutcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NeutcoinMiner(CWallet *pwallet)
{
    printf("NeutcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("neutcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NeutcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NeutcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NeutcoinMiner, pwallet));
}
